...
(function($){
	...
	var messages = {
		...
		errmsg: [
			...
			'Certaines questions sont restées sans réponse. Veuillez revenir en haut pour répondre à toutes les questions.'
		]
	}
	...
	$.quiz = $.fn.quiz = function(options)
	{
		...
		var defaults = {
			...
			modalBtnTpl: '<button class="' + BTN_CLASS + '" data-dismiss="modal">' 
				...
				+ '<%this.messages.restart%>' 
				+ '</button>',
			nextQuizBtnTpl: '<button class="' + BTN_CLASS + '">' +
    			'<%this.messages.nextQuiz%>' +
    			FORWARD_ICO +
    '</button>',
    ...
    // Append next quiz button to quiz buttons area
    $(TemplateEngine(settings.nextQuizBtnTpl, {
        'messages': messages
    })).appendTo('#' + BUTTONS_ID)

    // Event: Go to the next quiz
 // Event: Go to the next quiz
        .on('click', function(e) {
            e.preventDefault();
            ...
            // Déterminer la prochaine page en fonction de l'URL actuelle
            if (currentUrl.includes('quiz1.html')) {
                window.location.href = 'quiz2.html';
            }
            else {
				$.quiz.nextQuiz.textContent = "revenir à la page d'acceuil" ;
